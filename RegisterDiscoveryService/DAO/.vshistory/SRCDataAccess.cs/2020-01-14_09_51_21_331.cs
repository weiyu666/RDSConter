...
namespace RegistrationFoundService.Dal
{
    public class SRCDataAccess
    {
        ...
        public Dictionary<string, List<SRCMessage>> SRCSelect()//getlist  List<SRCMessage>
        {
            ...
            var resList = new  List<SRCMessage>();// List<SRCMessage>();
            try
            {
                lock (this)
                {
                    ...
                    foreach (DataRow item in data.Tables[0].Rows)
                    {
                        ...
                        if (!result.ContainsKey(message.service_name))
                        {
                            result[message.service_name] = new List<SRCMessage>();
                        }

                        result[message.service_name].Add(message);

                        // result.Add(Util.To<string>(item["service_name"]), resList);
                    }
                }
            }
